...
let AuthService = AuthService_1 = class AuthService {
    ...
    async verifyPassword(plainTextPassword, user) {
        const isPasswordMatch = await (0, bcrypt_1.compare)(plainTextPassword, user.password);
        if (!isPasswordMatch) {
    ...
    async toHashed(plainTextPassword) {
        ...
        return await (0, bcrypt_1.hash)(plainTextPassword, saltOrRounds);
    }
    ...
    async refreshToken(id, refreshToken) {
        ...
        const refreshTokenMatches = await (0, bcrypt_1.compare)(refreshToken, storeRefreshToken);
